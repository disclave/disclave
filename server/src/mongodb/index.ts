...
export const withSession = async <T>(run: (session: ClientSession) => T | Promise<T>): Promise<T> => {
  if (!_client)
    throw "Database connection is not initialized!";
...
export const withTransaction = async <T>(run: WithTransactionCallback<T>): Promise<T> => {
  return await withSession(async (session: ClientSession): Promise<T> => {
    // TODO: waiting for https://jira.mongodb.org/browse/NODE-2014
    let result: T;
